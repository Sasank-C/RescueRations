...
export class OpenaiService {
  public async validateImage(image: string): Promise<unknown> {
    ...
    const prompt = `
                    ...
                    {
                    'validityFactor': number, // Note: we are in testing phase so default to 1, 0-1, 1 if it satisfies all the criteria, 0 otherwise, 
                    'descriptionOfAnalysis': string, // indicate your analysis of the image and why it satisfies or not the criteria. The analysis will be shown to the user so make him understand why the image doesn't satisfy the criteria if it doesn't without going into detail on exact criteria. Remember we are rewarding users that drink coffee in a sustainable way. Note: we are in testing phase so default to 'Image is valid'
                    }
                    `;
